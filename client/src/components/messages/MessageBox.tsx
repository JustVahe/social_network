...
import { selectRoom, setMessagesOfTheRoom } from "../../redux/slices/roomSlice";
		...
		<div className="mt-6 sm:mt-0 col-span-5 sm:col-span-3 xl:col-span-4 relative">
			{
				optionsToggle && <div className="w-[150px] z-50 bg-zinc-100 rounded-md border border-zinc-300 absolute top-[-15px] right-[45px]">
					<button className="p-2.5 w-full rounded-md text-red-600 text-sm-14 transition hover:bg-red-600 hover:text-white">Delete Chat</button>
				</div>
			}
			<div className="w-full absolute top-0 flex gap-[15px] p-2.5 pt-0 border-b border-b-slate-200">
				{room.user_b ?
					...
					: <div>
						<img src={"/api/public" + room.avatar} alt="message_to_this_user" className="w-[45px] h-45px] object-cover object-top" />
						<div>
							<p className="text-sm-14 font-bold text-zinc-700">{room.name}</p>
						</div>
					</div>}
			</div>
