...
import { IFriend } from "../utils/types/types.ts";
...
export class FriendsService extends BaseService {
    ...
    async getFriendsByUserId(req: Request): Promise<any>{
        ...
        try {
            ...
            respectedFriends.map((item: IFriend) => {
    ...
    async createFriendship(req: Request): Promise<any>{
        ...
        try {
            ...
            if (!requestOfThisFriend) return this.response({ status: false, statusCode: 404, data: "Request of this friend wasn't found" })
    ...
    async unfriend(req: Request): Promise<any>{
        ...
        try {
            ...
            if (!friend) return this.response({ status: false, statusCode: 404, data: "Friend wasn't found" });
            friend.destroy();
