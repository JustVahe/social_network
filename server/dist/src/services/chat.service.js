...
export class ChatService extends BaseService {
    async createChat(req) {
        try {
            const { name } = req.body;
            if (!name)
                return this.response({ status: false, statusCode: 400, data: "Name is Required" });
    ...
    async getChat(req) {
        try {
            ...
            if (!chat)
                return this.response({ status: false, statusCode: 404, data: "Chat wasnt Found" });
