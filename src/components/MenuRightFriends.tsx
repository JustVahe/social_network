...
export default function MenuRightFriends() {
  return (
    <div className="w-[70px] h-svh bg-white fixed right-0 z-10 shadow-xl shadow-zinc-300 flex justify-center">
        <div className="mt-[90px] h-[570px] overflow-y-scroll no-scrollbar flex flex-col justify-between">
          {
            friends && friends.map(item => {
               return <FriendButton src={item.imageSrc} status={item.status} />
            })
          }
        </div>
    </div>
